...
def create_r_doc(func, package_name, skip):
    ...
    doc_str = re.sub(
        math_pattern, '(equation could not be rendered, see API doc on website)', doc_str
    )
...
def _create_r_params(doc_list, func, package_name, skip):
    ...
    params_unbound = [
        param for param in params if param.kind in (param.VAR_KEYWORD, param.VAR_POSITIONAL)
    ]
...
def _create_r_example(doc_list):
    if any('pharmpy-execute::' in row for row in doc_list):
        ...
        doc_code = [
            row for row in doc_list if row.startswith('>>>') or re.match(r'^\.\.\.\s+[$\w\d]', row)
        ]
